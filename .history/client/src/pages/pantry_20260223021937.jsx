...
export default function Pantry() {
  ...
  const [newIngredient, setNewIngredient] = useState(""); // added for the input
    ...
    try {
      ...
      if (res.ok) {
        ...
        const updated = await fetch(
          `http://localhost:3000/api/pantry?userId=${user.id}`
        );
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (res.ok) {
        // Update state immediately without needing full refresh
