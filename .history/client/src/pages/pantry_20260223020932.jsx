...
export default function Pantry() {
  ...
  const [newIngredient, setNewIngredient] = useState(""); // added missing state
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (res.ok) {
        ...
        // Optional: refetch from backend to double-check
        // const updated = await fetch(`http://localhost:3001/api/pantry?userId=${user.id}`);
        // const items = await updated.json();
        // setPantryItems(items);
      }
    } catch (err) {
  ...
  return (
    <div style={{ padding: "40px", minHeight: "100vh", backgroundColor: "#f5f5f5" }}>
      ...
      {/* Original page layout & comments preserved */}
